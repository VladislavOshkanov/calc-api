...
pub async fn get_age_experiences(State(state): State<Arc<AppState>>) -> Json<Vec<AgeExperience>> {
    let client = &state.db_client;
    let collection = client.database("openapi").collection::<AgeExperience>("age_experiences");

    let mut cursor = collection.find(None, None).await.expect("Failed to fetch age_experiences");
...
pub async fn delete_age_experience(
    ...
    match collection.delete_one(filter, None).await {
        Ok(result) if result.deleted_count > 0 => Ok(format!("AgeExperience with ID {id} deleted.")),
        Ok(_) => Err("AgeExperience not found.".to_string()),
